...
def ollama_client(original_function):
    """
    A decorator that injects a synchronous or asynchronous client (depending on the original function type)
    for interacting with the Ollama API. The client is initialized with a specified or default API base URL
    and passed to the wrapped function as a keyword argument.

    Args:
        original_function (Callable): The function to be wrapped, which will receive the Ollama client.

    Returns:
        Callable: The wrapped function with a synchronous or asynchronous client, depending on whether the
        original function is a coroutine.

    Raises:
        Exception: If an error occurs while initializing the client or calling the wrapped function, the
        exception is logged and re-raised.

    Decorators:
        wraps: Maintains the original function's metadata in the wrapper.

    Notes:
        - By default, the client connects to the Ollama API at `http://localhost:11434` unless an `api_base`
          argument is provided in the function's keyword arguments.
        - If the original function is asynchronous (a coroutine), an `AsyncClient` is injected. Otherwise, 
          a regular `Client` is used.
    """
...
async def ollama_embedding(*args, **kwargs) -> dict:
    """
    Asynchronously generates embeddings for a given input using the Ollama API.

    This function retrieves the Ollama `client`, `input`, and `model` from the keyword arguments and 
    calls the `embeddings` method on the client to obtain the vector representation of the input.

    Args:
        *args: Positional arguments passed to the function (not used in this case).
        **kwargs: 
            - client (AsyncClient): The Ollama API client, automatically injected by the `ollama_client` decorator.
            - input (str): The input text for which embeddings are to be generated.
            - model (str): The model to be used for generating the embeddings.

    Returns:
        dict: A dictionary containing the embeddings generated by the Ollama API for the input text.

    Raises:
        Exception: If an error occurs during the embedding generation, it is logged and re-raised.
    """
