...
1. Activate your virtual environment (if using a custom virtual environment)
...
3. Copy `config/.env/sample` into `config/.env` file and set environment variables accordingly. Check the [environment variables](#environment-variables) section 
for details.
4. Define the pipeline manifest for your REST API you're looking to extract data from. Check how to define an API pipeline manifest in 
...
5. Set the REST API key in a `config/secrets/api_key` file, or specify it using the `--api-key` as argument to the CLI
6. Get the base URL of your Qdrant Vector DB or deploy a local `Qdrant` ([Docs](https://qdrant.tech/documentation/)) vector database instance using docker:
...
7. Get your Gaianet node running ([Docs](https://docs.gaianet.ai/node-guide/quick-start)) or install Ollama ([Docs](https://ollama.com/)) provider locally. 
The latter is recommended if you're looking to run the pipeline on consumer hardware.
8. Load the LLM embeddings model of your preference into the LLM provider you chose in the previous step:
