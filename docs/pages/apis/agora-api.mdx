...
In order to consume data from this API, you are required to request an API key. Do not forget to set it under `config/secrets/api-key` 
or specify it directly using the `--api-key` argument in the CLI.
